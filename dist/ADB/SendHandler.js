...
function createFile(bulbMovement, colourOptionValue, customLampArray) {
    ...
    const singleLightBuilder = (onOff, bulb, brightness, color, saturation) => {
        ...
        return `{&quot;cct&quot;:0,&quot;color&quot;:${color},&quot;deviceName&quot;:&quot;${bulb}&quot;,&quot;deviceType&quot;:&quot;DEVICE_TYPE_HSL&quot;,&quot;isGroup&quot;:${bulb.length <= 4 ? true : false},&quot;isOff&quot;:${Number(brightness) === 0 ? true : false},&quot;lightness&quot;:${brightness},&quot;saturation&quot;:${saturation}}`;
    };
    const LightArrayBuilder = (fullrepeats) => {
        let fullString = "";
        for (let i = 0; i < fullrepeats; i++) {
            let tempString = singleLightBuilder(true, lampArray[getWalkingBulb()], brightness(), getRandomColor(), 100);
    ...
    return new Promise((resolve, reject) => {
        exec(`./adb root`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            if (stdout.includes("restarting adbd in root mode")) {
                exec(`./adb remount`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
...
let tries = 0;
const changeColor = (x, y) => __awaiter(void 0, void 0, void 0, function* () {
    let waiting;
    yield turnOnDisplay();
    yield sendTouchEvent(x, y, 0);
});
exports.changeColor = changeColor;
/////////////////////////////////////Sven's//Coding/ Date: 02-11-2022 17:28 ////////////
// starting an event
/////////////////////////////////////////gnidoC//s'nevS////////////////////////////////
const startEvent = () => __awaiter(void 0, void 0, void 0, function* () {
    const results = yield checkAndWait(649, 126, 66, "connected", 255, "disconneced", 211, "trying", 60);
    console.log(results);
});
...
const directControl = (event) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (scriptState !== "free") {
        __1.io.emit("serverStatus", `${updatedDateAndTime()}  Device is busy running another script`);
    }
});
...
/////////////////////////////////////////gnidoC//s'nevS////////////////////////////////
const checkAndWait = (x, y, color1, message1, color2, message2, color3, message3, timeOuts) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let result = "";
    const watchConnection = () => __awaiter(void 0, void 0, void 0, function* () {
        let pixelColor = yield checkPixelColor(x, y);
        yield betterSetTimeOut(2000);
        console.log(pixelColor);
        if (pixelColor[0] >= color1 - 2 && pixelColor[0] <= color1 + 2) {
            result = "finished";
            __1.io.emit("serverStatus", `${updatedDateAndTime()} ${message1}`);
        }
    ...
    for (let i = 0; i < maxTimeOuts; i++) {
        ...
        if (message1 === "connected" &&
            ...
            yield checkIfConnected();
            __1.io.emit("serverStatus", `${updatedDateAndTime()}  device succesfully restarted`);
            yield restartApp();
            __1.io.emit("serverStatus", `${updatedDateAndTime()}  restarting app`);
            yield root();
            __1.io.emit("serverStatus", `${updatedDateAndTime()}  kill OSRAM App`);
...
const turnOnDisplay = () => {
    return new Promise((resolve, reject) => {
        ...
        exec(`./adb shell "dumpsys display | grep "mScreenState""`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            if (error)
                reject(error);
            if (stdout.includes("mScreenState=OFF") ||
                stdout.includes("mScreenState=DOZE")) {
                exec(`./adb shell input keyevent KEYCODE_POWER`, { shell: "powershell.exe" }, (error, stdout) => {
                    console.log(stderr);
                    if (error) {
                        reject(error);
                    }
...
const checkPixelColor = (x, y) => {
    ...
    return new Promise((resolve, reject) => {
        exec(`./adb shell screencap screen.dump`, { shell: "powershell.exe" }, (error, stdout) => {
            if (error) {
                console.log(error);
            }
            exec(`./adb shell "dd if='./screen.dump' bs=4 count=1 skip=${pixelLocation} 2>/dev/null |  /sbin/.magisk/busybox/hd"`, { shell: "powershell.exe" }, (error, stdout) => {
                ...
                final = stdout
                    .substring(stdout.indexOf(pattern) + pattern.length, stdout.length)
                    .slice(0, 11);
...
/////////////////////////////////////////gnidoC//s'nevS////////////////////////////////
const sendTouchEvent = (x, y, timeOut) => __awaiter(void 0, void 0, void 0, function* () {
    return new Promise((resolve, reject) => {
        exec(`./adb shell input tap ${x} ${y}`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            setTimeout(() => {
                resolve(stdout);
            }, timeOut);
        });
...
/////////////////////////////////////////gnidoC//s'nevS////////////////////////////////
const checkIfConnected = () => __awaiter(void 0, void 0, void 0, function* () {
    let result = "notconnected";
    const tryPullFile = () => __awaiter(void 0, void 0, void 0, function* () {
        exec(`./adb pull "/XxX"`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            console.log(stdout);
            if (stdout.includes("no devices/emulators") ||
...
const RebootToDownload = () => __awaiter(void 0, void 0, void 0, function* () {
    if (scriptState === "free") {
        ...
        __1.io.emit("serverStatus", `${updatedDateAndTime()}  kill OSRAM App`);
        yield killApp();
        __1.io.emit("serverStatus", `${updatedDateAndTime()}  delete light file`);
        yield deleteFileFromAndroid();
        __1.io.emit("serverStatus", `${updatedDateAndTime()}  write new file`);
...
const writeFileToAndroid = () => {
    return new Promise((resolve, reject) => {
        exec(`./adb push C:/Users/server/Downloads/lightscript/lightscript/com.ledvance.smartplus.eu_preferences.xml "/data/data/com.ledvance.smartplus.eu/shared_prefs/"`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
...
/////////////////////////////////////////gnidoC//s'nevS////////////////////////////////
const reboot = (mode) => {
    exec(`./adb reboot ${mode}`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
...
const killApp = () => {
    return new Promise((resolve, reject) => {
        exec(`./adb shell "am force-stop com.ledvance.smartplus.eu"`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            resolve(stdout);
        });
    });
};
...
const restartApp = () => {
    return new Promise((resolve, reject) => {
        exec(`./adb shell "monkey -p com.ledvance.smartplus.eu -c android.intent.category.LAUNCHER 1"`, { shell: "powershell.exe" }, (error, stdout, stderr) => {
            resolve(stdout);
        });
    });
};
